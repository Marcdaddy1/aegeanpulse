...
- **`viewport.amount` must stay `"some"` — never a fraction.** A fractional threshold is unsatisfiable once an element is taller than `(1 / amount) x viewport`: at `0.2`, a 6,590px article body needs 1,318px on screen, which no phone can give. Blink tolerates this; **WebKit does not**, so content sat at `opacity: 0` forever on iOS. It shipped that way in Sept 2026 and made `/ai-news` articles look like they never loaded — on iOS *every* browser is WebKit, so "I tried two browsers" tests the same engine twice. Regression-test with Playwright's `webkit` + `devices['iPhone 13']`, not Chrome.
