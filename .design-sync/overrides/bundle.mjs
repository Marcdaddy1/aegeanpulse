...
// Repointed for the fork: esbuild lives in .ds-sync/node_modules, which is not
// on the resolution path from .design-sync/overrides/. Same install the
// bundled lib uses, so the two builds stay byte-identical apart from the alias.
import { build } from '../../.ds-sync/node_modules/esbuild/lib/main.js';
import { createHash } from 'node:crypto';
import { existsSync, readFileSync, statSync, writeFileSync } from 'node:fs';
import { dirname, join, resolve } from 'node:path';
// Repointed for the fork: this file loads from .design-sync/overrides/, so a
// './common.mjs' sibling import would not resolve. Reach back to the staged lib.
...
// ── AegeanPulse fork: alias next/link ────────────────────────────────────
// ONLY deviation from the bundled lib/bundle.mjs. Button imports next/link,
// which drags Next's client router into the bundle; that router reads
// process.env.__NEXT_* at module scope, so every component threw
// "ReferenceError: process is not defined" in the render check and would have
// thrown the same in every design the agent builds.
//
// tsconfig paths can't fix it: esbuild auto-discovers the REPO tsconfig for
// files under src/ and ignores the one passed via cfg.tsconfig, and adding the
// alias to the repo tsconfig would redirect next/link in the real Next.js app.
// esbuild's own `alias` option is the narrow fix, and it isn't reachable from
// config — hence this fork.
//
// Button passes Link only href + className, so the plain <a> in the shim is
// behaviourally identical for a standalone render. The shipped .d.ts still
// documents href, so the component's API is unchanged.
function nextLinkAlias(nodePaths) {
  const shim = join(nodePaths, 'aegeanpulse', 'ds-shims', 'next-link.tsx');
  return existsSync(shim) ? { 'next/link': shim } : undefined;
}
